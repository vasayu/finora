...
class AIService {
     ...
     */
    async analyzeFinancials(userId, query, context) {
...
Provide a detailed financial analysis addressing the user's query based on this data.`;
        try {
            ...
            logger_1.default.error('OpenAI analysis error:', error);
    ...
    async chat(userId, message, conversationHistory) {
        ...
        const recentTx = await database_1.prisma.transaction.findMany({
            where: { userId },
            orderBy: { date: 'desc' },
            take: 20,
        });
        ...
        try {
            ...
            logger_1.default.error('OpenAI chat error:', error);
