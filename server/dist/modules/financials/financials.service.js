...
class FinancialsService {
    async getPnL(userId, organizationId, startDate, endDate) {
     ...
     */
    async getBalanceSheet(userId, organizationId, asOfDate) {
        ...
        const balances = {};
        for (const item of chart_of_accounts_1.CHART_OF_ACCOUNTS) {
        ...
        for (const tx of transactions) {
            if (tx.type === 'INCOME') {
                ...
                const account = (0, chart_of_accounts_1.classifyTransaction)(tx.category, 'INCOME');
                ...
                // EXPENSE
                totalExpense += tx.amount;
                const account = (0, chart_of_accounts_1.classifyTransaction)(tx.category, 'EXPENSE');
                if (account === 'Cash & Cash Equivalents') {
                    ...
                    // and cash decreases
                    const targetItem = chart_of_accounts_1.CHART_OF_ACCOUNTS.find(i => i.account === account);
        ...
        for (const section of sections) {
            const sectionItems = chart_of_accounts_1.CHART_OF_ACCOUNTS.filter(i => i.section === section);
     ...
     */
    async exportBalanceSheetExcel(userId, organizationId, asOfDate) {
        const dateLimit = asOfDate ? new Date(asOfDate) : new Date();
        dateLimit.setHours(23, 59, 59, 999);
        const where = organizationId
            ? { organizationId, date: { lte: dateLimit } }
        ...
        for (const tx of transactions) {
            const bsAccount = (0, chart_of_accounts_1.classifyTransaction)(tx.category, tx.type);
            txSheet.addRow({
                date: tx.date,
                type: tx.type,
        ...
        const lineItemRows = {}; // track row numbers for subtotals
        const addLineItem = (account, section) => {
        ...
        const addTotalRow = (label, subtotalRefs) => {
            const row = bsSheet.addRow(['', label, null]);
            row.getCell(3).value = {
